...
def getAssignmentList(session, semester_list, semester):
    ...
    for semester_subject in semester_list[semester][2:]:
        ...
        response = session.get(
            f"https://binusmaya.binus.ac.id/services/ci/index.php/student/classes/assignmentType/{CRSE_CODE}/{CRSE_ID}/{STRM}/{SSR_COMPONENT}/{CLASS_NBR}/01",
            headers=headers,
        )
        ...
        print(
            f"[+] Getting assignment on {semester_subject['COURSE_TITLE_LONG']}, ({semester_subject['SSR_COMPONENT']})"
        )
        if assignment_list_temp != []:
            for assignment in assignment_list_temp:
                print(colored(f"[!] {assignment['Title']}", "yellow"))
                print(
                    f" └─ Due date    : {assignment['deadlineTime']}, {assignment['deadlineDuration']}"
                )

                data = {"id": str(assignment["StudentAssignmentID"])}
                response = session.post(
                    "https://binusmaya.binus.ac.id/services/ci/index.php/student/classes/getHistoryAssignment",
                    json=data,
                    headers=headers,
                )
                ...
                for submission in submission_list:
                    print(
                        colored(
                            f"    [!] You already submit the answer on {submission['submittedDate']} ({submission['Title']})",
                            "green",
                        )
                    )
            ...
            print(
                f"[-] There is no assignment on {semester_subject['COURSE_TITLE_LONG']}, ({semester_subject['SSR_COMPONENT']})"
            )
    print()
    ...
    for semester_subject in semester_list[semester][2:]:
        ...
        headers = {"referer": "https://binusmaya.binus.ac.id/newStudent/"}
        response = session.get(
            f"https://binusmaya.binus.ac.id/services/ci/index.php/student/classes/assignmentType/{CRSE_CODE}/{CRSE_ID}/{STRM}/{SSR_COMPONENT}/{CLASS_NBR}/02",
            headers=headers,
        )
        # https://binusmaya.binus.ac.id/services/ci/index.php/student/classes/basicinfo/010550/2010/LAB
...
def writeAssignmentToMarkdown(
    storage_path, assignment_list, assignment_subject, assignment_complete
):
        ...
        for index, subject in enumerate(assignment_subject):
            ...
            for assignment in assignment_list[index]:
                if assignment in assignment_complete:
                    f.write(
                        f"[x] {assignment['Title']}, deadline {assignment['deadlineTime']}, {assignment['deadlineDuration']}\n"
                    )
                else:
                    f.write(
                        f"[ ] {assignment['Title']}, deadline {assignment['deadlineTime']}, {assignment['deadlineDuration']}\n"
                    )
...
def writeAssignmentToMarkdownForNotion(
    ...
    print(
        colored(
            "[+] Successfully writing all assignment to assignment_notion.md", "green"
        )
    )
    print()
