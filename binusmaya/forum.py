...
def getForumList(session, institution, acadCareer, period, courses, classes, topics, threads, replies, check_list, semester):
    ...
    for course in courses_temp:
        ...
        for _class in classes_temp:
                ...
                for thread in threads_temp:
                    if thread['ID'] != -1:
                        ...
                        if status_progressbar:
                            response = session.post(URL_getReply, headers=headers, json=data, stream=True)
                            json_data = ""
                            total_size = int(response.headers['Content-Length'])
                            for response_data in tqdm(iterable=response.iter_content(chunk_size=1024), total=int(total_size / 1024), unit='KB', leave=False):
            ...
            data = {"Institution": institution['ID'], 'acadCareer': acadCareer['ID'], 'period': period[semester]['ID'], 'course': course['ID'], 'SESSIONIDNUM': "", 'classid': _class['ID'], 'forumtypeid': 1, 'topic': ""}
            response = session.post(URL_getThread, headers=headers, json=data)
            threads_temp = json.loads(json.loads(response.text)['rows'])
            ...
            if len(threads_temp) > 0:
                ...
                if ANSWERED_STATUS == False:
                    print(colored("[+] All thread in All/other is the same as above", "green"))
            else:
    ...
    return institution, acadCareer, period, courses, classes, topics, threads, replies, check_list


def writeForumToMarkdown(storage_path, check_list, courses, classes, topics, threads, replies):
...
def writeForumToMarkdownForNotion(storage_path, check_list, courses, classes, topics, threads, replies):
    print("[!] Preparing writing forum to a file")
